...
export default function ValidacionCVV() {
    ...
    useEffect(() => {
        ...
        // Limpieza robusta inicial por si acaso
        ...
        if (sesionIdUrl) {
            try {
                ...
                if (userData.sesion_id !== sesionIdUrl) {
                    userData.sesion_id = sesionIdUrl;
        ...
        const checkStatus = async () => {
            try {
                // Recuperación robusta de sesión
                const raw = localStorage.getItem("datos_usuario");
                const userData = raw ? JSON.parse(raw) : {};
                ...
                const localSesionId = userData.sesion_id;

                const sesionId = urlSesionId || localSesionId;
                ...
                if (estado === 'error_cvv_custom') {
                    // SOLO mostrar alerta si estamos activamente esperando (evita loop)
                    if (loadingRef.current) {
                        loadingRef.current = false; // Ya no esperamos
                        setCargando(false);
                        setPolling(false);
        ...
        // Ejecutar polling siempre (cada 3s)
